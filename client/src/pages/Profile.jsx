        ...
        <>
          <div className='flex justify-center items-center min-h-screen'>
            <div className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-[90%] sm:w-[50%] lg:w-[30%]'>
              <h1 className='text-3xl sm:text-4xl text-center mb-8 tracking-wider text-blue-950 font-bold dark:text-indigo-100'>Profile</h1>
              <form onSubmit={handleSubmit} className='flex flex-col items-center'>
                <input type='file' onChange={(e) => setFile(e.target.files[0])} ref={fileRef} hidden accept='image/*' />
                <Avatar className='w-24 h-24 mb-6 cursor-pointer' onClick={() => { fileRef.current.click() }}>
                  <AvatarImage src={formData.avatar || user.avatar} alt="profile picture" />
                  <AvatarFallback>Img</AvatarFallback>
                </Avatar>
                <p className='mb-6'>
                  {fileUploadError ? (<span>Error image upload</span>) : fileper > 0 && fileper < 100 ? (<span>{`uploading ${fileper}%`}</span>) : fileper === 100 ? (<span className='text-green-600'>Image successfully uploaded</span>) : null}
                </p>
                <input defaultValue={user.name} id='name' type="name" required onChange={handleChange} className='py-2 px-3 border border-slate-500 dark:bg-transparent dark:border-slate-400 dark:text-white rounded-md mb-6 w-full' />
                <input defaultValue={user.email} id='email' type="email" required onChange={handleChange} className='py-2 px-3 dark:bg-transparent dark:text-white border border-slate-600 dark:border-slate-400 rounded-md mb-6 w-full' />
                <button className='border px-6 py-2 rounded-lg bg-violet-500 text-white dark:border-violet-500 w-full mb-4'>Update</button>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant='outline' className='border px-6 py-2 rounded-lg border-red-500 hover:bg-red-600 hover:text-white dark:border-red-500 dark:hover:bg-red-500 dark:bg-transparent dark:text-white w-full'>Delete account</Button>
                  </PopoverTrigger>
