  ...
  return (
    <main>
      <div className="flex items-center justify-center flex-col space-y-2">
        {loading && (
          <p className="flex items-center justify-center mt-[50%] md:mt-[15%] animate-spin dark:text-slate-300">
            <Loader2Icon />
          </p>
        )}
        {error && (
          <>
            <p className="flex items-center text-black justify-center mt-[50%] md:mt-[15%] dark:text-slate-300">
              Something went wrong...
            </p>
            <Link
              to="/"
              className="px-5 py-2 border border-slate-700 rounded-lg dark:border-slate-300 dark:text-slate-200"
            >
              Go to Home
            </Link>
          </>
      ...
      <div>
        {issue && !loading && !error && (
          <div>
            ...
            <p className="mx-4 font-light text-lg mb-4 dark:text-slate-400 max-w-5xl">
              {issue.description}
            </p>
            <p className="mx-4 mb-4">Priority-{issue.priority}</p>
            <p className="mx-4 mb-4">Status-{issue.status}</p>
            <button className="py-1 px-5 text-sm bg-green-300 dark:bg-green-400 mx-4 rounded-2xl dark:text-slate-700">
              {issue.label}
            </button>
          </div>
        )}
